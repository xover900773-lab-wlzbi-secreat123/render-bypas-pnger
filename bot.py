...
class PingEngine:
    ...
    async def ping_url(self, record: URLRecord) -> Tuple[str, Optional[int], Optional[int]]:
        ...
        if self.session is None or self.semaphore is None:
            logger.error("Ping engine is not initialized")
            return record.id, None, None
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    text = (
        "🤖 URL PINGER\n"
        "━━━━━━━━━━━━━━━━━━━\n"
        "Welcome to URL Pinger.\n"
        f"Keep your URLs monitored and automatically pinged every {PING_INTERVAL} seconds.\n"
        "━━━━━━━━━━━━━━━━━━━\n"
    )
    ...
    try:
        while True:
            try:
                ...
                logger.exception("Error in ping scheduler iteration")
    finally:
        if ping_engine:
...
@health_app.get("/")
def health_check():
    return "URL Pinger Bot is running", 200


def run_health_server():
    health_app.run(host="0.0.0.0", port=int(os.getenv("PORT", "10000")), threaded=True, use_reloader=False)
...
def build_application() -> Application:
    ...
    conv_handler = ConversationHandler(
        entry_points=[CallbackQueryHandler(user_callback, pattern="^add$")],
        states={
    ...
    app.add_handler(CallbackQueryHandler(user_callback, pattern="^(my_urls|ping_url|my_stats|help|back|user_menu)$"))
